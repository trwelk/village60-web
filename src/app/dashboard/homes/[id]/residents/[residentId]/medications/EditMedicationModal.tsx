...
export type EditMedicationTarget = {
  id: string;
  itemId: string;
  name: string;
...
type Props = {
  homeId: string;
  residentId: string;
  medication: EditMedicationTarget | null;
  onClose: () => void;
};
...
function formFromMedication(medication: EditMedicationTarget) {
  return {
    itemId: medication.itemId,
    quantityPerServing: String(medication.quantityPerServing),
    directions: medication.directions,
    servingsPerDay:
      medication.servingsPerDay == null ? "" : String(medication.servingsPerDay),
    prn: medication.prn,
    scheduledSlots: medication.prn
      ? ([] as MarTimeSlot[])
      : medication.scheduledSlots.length > 0
        ? [...medication.scheduledSlots]
        : ([...MAR_TIME_SLOTS] as MarTimeSlot[]),
  };
}
...
export function EditMedicationModal({
  ...
  const [form, setForm] = useState(() =>
    medication ? formFromMedication(medication) : formFromMedication({
      id: "",
      itemId: "",
      name: "",
  ...
  async function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!medication || !form.itemId || submitting) return;
    ...
    try {
      ...
      const res = await fetch(
        `/api/homes/${homeId}/residents/${residentId}/clinical/medications/${encodeURIComponent(medication.id)}`,
        {
          method: "PATCH",
  ...
  return createPortal(
    <div className="fixed inset-0 z-[200] flex items-end justify-center p-0 pb-[env(safe-area-inset-bottom,0px)] sm:items-center sm:p-6 sm:pb-6">
      <button
        ...
        aria-label="Dismiss edit medication dialog"
        onClick={closeModal}
      />
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="medications-edit-modal-heading"
        data-testid="medications-edit-panel"
          ...
          <section className="village-card overflow-hidden border-0 p-0 shadow-none">
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div className="flex max-w-2xl gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <h2
                      id="medications-edit-modal-heading"
                      className="text-xl font-semibold tracking-tight text-pine-2"
                    >
                      Edit medication
                    </h2>
                    <p className="text-sm leading-6 text-ink/65">
                      Update dose, directions, and schedule for{" "}
                      <span className="font-semibold text-pine-2">{medication.name}</span>.{" "}
                      <Link
            ...
            >
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap">
                ...
                <label
                  htmlFor="medications-edit-servings"
                  className="flex min-w-[10rem] flex-1 flex-col gap-2"
                >
                  <span className="village-label">Servings per day (optional)</span>
                  <input
                    id="medications-edit-servings"
                    className="village-input min-w-0"
              ...
              <label htmlFor="medications-edit-directions" className="flex flex-col gap-2">
                <span className="village-label">Directions</span>
                <textarea
                  id="medications-edit-directions"
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                <button
                  form="medications-edit-form"
                  type="submit"
                  className={MODAL_PRIMARY_BTN_CLASS}
                  disabled={submitting || loadingItems || items.length === 0 || !form.itemId}
                >
