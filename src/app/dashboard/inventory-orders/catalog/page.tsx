...
import { HomeItemCatalogClient } from "./HomeItemCatalogClient";

type PageParams = {
  searchParams?: Promise<{ homeId?: string }>;
};

export default async function HomeItemCatalogPage({ searchParams }: PageParams) {
  ...
  return (
    <main className="flex flex-col gap-8 text-[var(--text-primary)]">
      <HomeItemCatalogClient homes={homes} selectedHomeId={selectedHomeId} />
    </main>
  );
}
