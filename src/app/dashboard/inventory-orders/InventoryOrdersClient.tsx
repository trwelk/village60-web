...
export function InventoryOrdersClient({
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter((o) => {
      if (
        statusFilter !== "all" &&
        o.status.trim().toUpperCase() !== statusFilter
      ) {
        return false;
      }
      ...
      <VillageList
        ...
        filters={
          <>
            {homes.length > 1 ? (
              ...
              >
                <VillageSelect
                  ...
                  onChange={(nextId) =>
                    router.push(
                      `/dashboard/inventory-orders?homeId=${encodeURIComponent(nextId)}`,
                    )
                  }
                  options={homes.map((h) => ({
                    value: h.homeId,
              ...
              <VillageListFilter
                label="Home"
                htmlFor="inventory-orders-home-ro"
              >
                <input
                  id="inventory-orders-home-ro"
            ...
            <VillageListFilter
              label="Status"
              htmlFor="inventory-orders-status"
              width="11rem"
            >
              <VillageSelect
                id="inventory-orders-status"
                value={statusFilter}
            ...
            <VillageListFilter
              label="Search"
              htmlFor="inventory-orders-search"
              minWidth="12rem"
            >
              <input
                    ...
                    filteredOrders.map((order) => (
                      ...
                      >
                        ...
                        <td className="whitespace-nowrap px-5 py-4 font-mono text-xs tabular-nums text-[var(--text-secondary)]">
                          {formatPurchaseOrderCreatedAt(
                            order.createdAtUtcMs,
                            appTimezone,
                          )}
                        </td>
                        ...
                        <td className="px-5 py-4 text-right font-semibold tabular-nums text-[var(--text-primary)]">
                          {order.currencyCode &&
                          order.totalReceivedCents > 0 ? (
                            formatCents(
                              order.totalReceivedCents,
                              order.currencyCode,
                            )
                          ) : (
        ...
        ? createPortal(
            <div className="fixed inset-0 z-[200] flex items-end justify-center p-0 pb-[env(safe-area-inset-bottom,0px)] sm:items-center sm:p-6 sm:pb-6">
              ...
              <div
                ...
                className="relative z-10 flex max-h-[min(calc(100dvh-env(safe-area-inset-bottom,0px)-0.75rem),52rem)] w-full min-h-0 max-w-4xl flex-col overflow-hidden rounded-t-2xl border border-[color:color-mix(in_srgb,var(--line-strong)_50%,transparent)] bg-[color:color-mix(in_srgb,var(--bg-muted)_35%,var(--bg-elevated)_65%)] shadow-[0_-8px_40px_-12px_color-mix(in_srgb,var(--text-primary)_35%,transparent)] sm:max-h-[min(92dvh,56rem)] sm:rounded-2xl sm:shadow-[0_22px_60px_-24px_color-mix(in_srgb,var(--text-primary)_38%,transparent)]"
              >
                        ...
                        <div className="flex max-w-2xl gap-4">
                          ...
                          <div className="flex flex-col gap-1">
                            ...
                            <p className="text-sm leading-6 text-ink/65">
                              Choose a supplier and create a draft PO for this
                              home.
                            </p>
                          </div>
                        </div>
