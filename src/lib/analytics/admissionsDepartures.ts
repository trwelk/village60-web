...
export type DepartureReasonSlice = {
  reason: string;
  count: number;
  percent: number;
};

export type DepartureReasonBreakdown = {
  slices: DepartureReasonSlice[];
  distinctReasonCount: number;
  totalDeparturesInRange: number;
};

/**
 * Departure reasons for `resident_departure_details` in the rolling 12 UTC months
 * ending at `atUtcMs`, including zero-count months boundary on departures only.
 */
export function getDepartureReasonBreakdownLastTwelveMonths(
  db: AppDb,
  atUtcMs: number,
): DepartureReasonBreakdown {
  const endMonth = utcBillingMonthFromMs(atUtcMs);
  const startMonth = shiftBillingMonth(endMonth, -11);
  const { startMs } = utcMonthRangeExclusiveEnd(startMonth);
  const { endExclusiveMs } = utcMonthRangeExclusiveEnd(
    shiftBillingMonth(endMonth, 1),
  );

  const rows = db
    .select({
      reason: residentDepartureDetails.reason,
      c: count(),
    })
    .from(residentDepartureDetails)
    .where(
    ...
    .groupBy(residentDepartureDetails.reason)
    .all();

  const total = rows.reduce((acc, r) => acc + Number(r.c), 0);
  const slices: DepartureReasonSlice[] = rows
    .map((r) => {
      const c = Number(r.c);
      const pct = total > 0 ? Math.round((100 * c) / total) : 0;
      return {
        reason: r.reason.trim() === "" ? "(unspecified)" : r.reason.trim(),
        count: c,
        percent: pct,
      };
    })
  ...
  return {
    slices,
    distinctReasonCount: slices.length,
    totalDeparturesInRange: total,
  };
}
